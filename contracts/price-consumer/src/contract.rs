...
const CONTRACT_NAME: &str = "crates.io:price-consumer";
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> Result<QueryResponse, ContractError> {
    match msg {
        ...
        QueryMsg::LatestRoundData {} => Ok(to_binary(&query_latest_round(deps)?)?)
    }
}
...
fn query_latest_round(deps: Deps) -> Result<Round, ContractError> {
    let config = CONFIG.load(deps.storage)?;
    let round = deps
    .querier
    .query_wasm_smart(config.feed, &ChainlinkQueryMsg::LatestRoundData)?;
